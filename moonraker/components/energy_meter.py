# Energy Meter - process energy data from sensors
# and and provide delta measurements
#
...
class EnergyMeter:
    ...
    def __init__(self, config: ConfigHelper) -> None:
        ...
        self.server.register_event_handler("sensors:sensor_update",
                                           self._on_sensor_update)

    def _update_values(self, sensor_values: Dict[str, Union[int,
                                                            float]]) -> None:
        """
        ...
        if self.config.field not in sensor_values:
            logging.error(f"Energy value field '{self.config.field}'"
                          "not in data of sensor: '{self.config.sensor}'")
            return None
        value = sensor_values[self.config.field]
        if self.active_msmnt.last_value is not None and \
                value < self.active_msmnt.last_value:
            logging.error("EnergyMeter must not go down: %d -> %d",
    ...
    def _on_sensor_update(
            self, changed_data: Dict[str, Dict[str, Union[int,
                                                          float]]]) -> None:
        """
    ...
    def start_measurement(self) -> Optional[DeltaMeasurement]:
        ...
        msmnts: Dict[str, Union[int, float]] = {
            key: values[0]
            for key, values in sensor.values.items()
        }
    ...
    async def initialize(self) -> bool:
        ...
        """
        logging.info("Registered EnergyMeter for sensor '%s'",
                     self.config.sensor)
        return True
